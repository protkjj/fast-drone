...
## 발표 플롯 세션 (2026-07-29) — 시나리오 A 스윕 재실행 + PNG 생성

### 배경
legacy/sweep.py·sweep_scheduled.py는 텍스트 출력만 하고 플롯 미저장 → 7월 발표용
(단계 4: 성능 저하 + 게인 스케줄링, NMPC 비공개)으로 시나리오 A만 재실행.

### 산출물 (신규 진입점: `sweep_plots.py`, 실행 ~40초)
- `results/sweep_knee.png/.txt` — 고정 게인 저하 곡선 (RMSE z, 요청 사양)
- `results/sweep_knee_vxz.png` — vx+z 2패널 확장판 (진짜 스토리는 여기)
- `results/sweep_fixed_vs_scheduled.png/.txt` — 고정 vs 스케줄 4곡선 (RMSE z)
- `results/sweep_fixed_vs_scheduled_vxz.png` — vx+z 2패널 확장판

### 핵심 발견 (기대와 달랐음 — 검증 완료, 버그 아님)
1. **정적 순항 회복에서 LQR은 RMSE z 저하가 전혀 없음** (0.30→0.25로 오히려 감소).
   공력 감쇠 dD/dv ∝ V 덕분에 고속 회복이 더 쉬움 (오픈루프 rmse_vx 2.54→0.69 감소로 확인).
2. **고정 게인 저하의 주범은 게인이 아니라 트림 피드포워드**: K_r은 30↔85 m/s 간
   52% 다르지만 게인만 고정 시 저하 ~0. 트림까지 고정(엄격 고정)하면
   RMSE vx 0.86→4.93 (5.7배, 무릎점 70 m/s) — 저하가 z가 아닌 **vx 축**에 나타남.
3. PID 고정은 z·vx 모두 저하 (z 0.40→2.40, 무릎점 70 m/s). 단 ScheduledPID는
   z만 ~13% 개선하고 vx는 오히려 악화 (Kp_vel 축소 설계 의도상 당연).
4. 85 m/s 고정/스케줄 비율: LQR vx **8.0배** (4.93/0.62), z 1.1배 / PID z 1.15배.
   → "고정↑ vs 스케줄 평평" 대비는 **vx 패널 + 엄격 고정 LQR**에서만 극적.
5. ⚠️ **legacy/sweep.py의 고정 LQR은 현재 controller.py와 비호환**: `__new__`로 `.K`만
   설정하는데 리팩터링된 `__call__`은 `.K_r` 사용 → 재실행 시 전 속도 DIV (조용한 잠복).
6. ScheduledLQR 기본 V_table(0~80)은 85 m/s에서 80 트림을 목표로 삼는 인공 오차 발생
   → sweep_plots.py는 테이블에 85 추가.

### 남은 것
- 발표 슬라이드에 z 단일축 vs vx+z 2패널 중 택일 (kj 결정)
- 고정 게인의 "발산" 수준 실패는 시나리오 B(천이, 단계 5)에서 나옴 — 다음 주 범위

## 검증 세션 (2026-08-03) — 필기노트 vs 코드 대조 + 공유 준비

- **동역학 모델링 = 필기노트와 일치 확인** (지배방정식·로터·공력·할당·트림·호버 전 항목).
  θ 부호만 규약 차이: 노트는 전방틸트 양수(θ≈-α), 코드는 기수상승 양수(트림 θ=α, 수평비행 θ=α+γ, γ=0)
- test_plant.py 6/6 통과, 전체 .py 문법 OK, 트림 잔차 ~1e-12 (V=0: θ=Δn=0, n_eq=571.8=해석해)
- .gitignore에 CLAUDE.md 추가 (단, 이미 추적 중 → `git rm --cached` 필요, kj 승인 대기)
- 발견: test_plant.py:6 절대경로 하드코딩 (공유 시 제거 권장), `model 2.sdf`=구버전 스냅샷 (정리 대상)
