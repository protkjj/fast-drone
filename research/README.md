...
STL(현재 기본): `assets/drone_hsd.stl`, 원본 SHA-256
`70c466080b72c836f9e520f75a0cb06183fc9003813512a1c7148ff733792cfd`.
형상팀 `HSD_drone_assembly.step`(Fusion 360, 2026-09-21, 51개 솔리드)을 FreeCAD
2mm 테셀레이션으로 변환(211,220개 삼각형). 동체 길이·지름·로터 암 반경이
`profiles/selected.json`의 CSV 사이징 값과 mm 단위로 이미 일치해(길이 오차
<1mm) 별도 치수 변형 없이 그대로 쓴다 — 기수 X=0, CG=기수에서 428.251 mm로
변환하는 규약은 기존과 동일. 재질/밀도는 51개 중 1개에만(강철, 기본값으로
보임) 있어 질량·관성은 이 STL/STEP에서 계산하지 않고 여전히 CSV 사이징
값만 쓴다. 라디얼 100mm 기준으로 동체·암 마운트와 로터(모터벨 이후) 4묶음을
나눠 export했다.

STL(레거시, 회귀 테스트·감사 이력 보존용): `assets/drone_v2.stl`, 원본 SHA-256
...
`assets/selected_geometry.json`의 출처가 명시된 치수로 핀·하우징·로터 배치를
변환한 `alignSelectedGeometry()`는 이 STL 전용이며(CSV 형상과 어긋나 있어
맞춤 변형이 필요했음), 더는 연구 페이지의 기본 경로가 아니다 — drone_hsd.stl은
이미 CSV와 일치해 변형이 필요 없다. 동체 표면과 블레이드·허브 세부 형상은
기존 STL을 재사용하며 제조용 CAD나 CFD 모델이 아니다. 원본 STL, CG·관성·CP·
추진 모델은 이 표시 변환으로 수정하지 않는다.

두 표시 메시 모두: 화면의 회전은 실행 중 RPM 상태를 읽되 최대 약 2.9회/초로
시각적 감속하고 옅은 회전 잔상을 추가한다. 실제 RPM·추력·토크·적분 위상에는
영향을 주지 않는다.
