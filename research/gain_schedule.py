...
# 예산 120평가·본시험 비참조). 이력은 research/generated/tuned_gains.json.
# 튜닝 전 placeholder: Q diag [100,10,10,20, 50,50,50, 5,5,5, .01x4], R 0.05I.
# 바뀐 것은 q_z x0.5 · q_v_h x64 · r x0.25 뿐이고 나머지는 그대로다. 수평 속도
# 오차에 큰 가중치를 주고 입력을 싸게 만든 방향이며, 튜닝 시나리오에서 실패
# 3건 -> 0건이 됐다. 대신 고도 추종이 나빠졌다(z RMSE 0.67 -> 4.19) — 목적함수가
# v와 z를 같은 무게로 더하는데도 그렇게 갈린 것이므로, 이 기체에서 두 축이
# 상충한다는 뜻이다. §5.3 이 본시험을 보고 가중치를 다시 고르는 것을 금지하므로
# 손으로 고치지 말 것.
TUNED_Q_DIAG = [50, 640, 640, 20, 50, 50, 50, 5, 5, 5, .01, .01, .01, .01]
TUNED_R_SCALE = .0125
