...
function controlHybrid(x, cmd){
  ...
  const A = D.alloc_inv, nOut = [0,0,0,0];
  for (let i=0;i<4;i++){
    let Ti = A[i][0]*Tc + A[i][1]*M[0] + A[i][2]*M[1] + A[i][3]*M[2];
    if (Ti < 0) Ti = 0;
    const ni = Math.sqrt(Ti / P.k_T);
    nOut[i] = ni > P.n_max ? P.n_max : ni;
  }
  return nOut;
}
...
function initUI(){
  ...
  $("#expCsv").addEventListener("click", () => {
    const b = new Blob([scoreCsv()], {type:"text/csv;charset=utf-8"});
    const a = document.createElement("a");
    a.href = URL.createObjectURL(b);
    a.download = "flight_score_" + CTRL + "_" + Math.round(T) + "s.csv";
