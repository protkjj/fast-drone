...
.warnbox{position:absolute; left:14px; bottom:12px; right:14px;
...
function diag(){
  ...
  return {V, alpha, q_bar, F:Math.hypot(Fx,Fy,Fz), tilt,
          gs:Math.hypot(X[3],X[4]), alt:X[2]};
}

/* ══ 3D ═══════════════════════════════════════════════════════════════ */
let ren, scene, cam, veh, arrow, trail, trailPos, trailN = 0;
function init3D(){
  const host = $("#view");
  ...
  const skin = new THREE.MeshStandardMaterial({color:0x2b3440, roughness:.55, metalness:.3});
  const tip = new THREE.MeshStandardMaterial({color:0xd8402f, roughness:.45});
  veh = new THREE.Group();
  const b = new THREE.Mesh(new THREE.CylinderGeometry(.075,.075,.78,26), skin);
  b.rotation.z = -Math.PI/2; veh.add(b);
  const c = new THREE.Mesh(new THREE.ConeGeometry(.075,.24,26), tip);
  c.rotation.z = -Math.PI/2; c.position.x = .51; veh.add(c);
  for (const [dx,dy] of [[.177,.177],[.177,-.177],[-.177,-.177],[-.177,.177]]){
    const a = new THREE.Mesh(new THREE.CylinderGeometry(.012,.012,.25,10), skin);
    a.rotation.z = Math.PI/2; a.position.set(dx/2, dy/2, 0);
    a.lookAt(new THREE.Vector3(dx, dy, 0)); veh.add(a);
    const r = new THREE.Mesh(new THREE.CylinderGeometry(.15,.15,.012,18), skin);
    r.position.set(dx, dy, -.03); r.rotation.x = Math.PI/2; veh.add(r);
  }
...
function render3D(d){
  ...
  const W = P.mass * P.g;
  if (d.F > 1e-3){
    const R = new THREE.Matrix4().makeRotationFromQuaternion(veh.quaternion);
    arrow.visible = true; arrow.position.copy(p);
    // 화살표는 표시용이라 동체 좌표 힘을 월드로 돌려 방향만 쓴다
    const fw = new THREE.Vector3(1,0,0).applyMatrix4(R);
    arrow.setDirection(fw.normalize());
    const L = Math.max(2, 16*d.F/W);
    arrow.setLength(L, L*.22, L*.13);
  } else arrow.visible = false;

  const vdir = new THREE.Vector3(X[3], X[4], 0);
  if (vdir.lengthSq() < 1e-4) vdir.set(1,0,0);
  vdir.normalize();
  const up = new THREE.Vector3(0,0,1);
  const side = new THREE.Vector3().crossVectors(vdir, up).normalize();
  cam.position.copy(p).addScaledVector(vdir, -17).addScaledVector(side, 13)
