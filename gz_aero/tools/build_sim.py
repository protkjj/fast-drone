...
const CTRLS = {
  ...
  sqprti: {name:"SQP-RTI + INDI · 실험", group:"실험 제어기", ff:true, tilt:80, indi:true, nmpc:true, rti:true,
    note:"매 제어 주기 한 번의 SQP 반복을 수행합니다. 속도 추종과 구동기 여유 사이의 타협을 채점에서 확인하세요."},
};
let CTRL = "lqr";
...
function scoreCsv(){
  ...
  const rows=REC.slice(0,curIdx()+1).map(r=>[r.t,r.seg,r.cmd,r.V,r.x[2],r.zr,r.al,
    ...
    ...COEFS.map(([k])=>r.config.params[k])].map(v=>v===null?'':typeof v==='number'?+v.toFixed(6):v).join(','));
...
function initUI(){
  ...
  const sel = $("#ctrl");
  sel.innerHTML = ['기준 비행','비교 실험','실험 제어기'].map(group=>
