...
/* 1120 -> 1280. 차트가 2 칸으로 바뀌어 폭이 더 필요하다.
   본문 문단은 아래에서 66ch 로 따로 묶으므로 글줄은 안 길어진다. */
...
/* text-wrap:pretty — 마지막 줄에 '다.' 한 글자만 남는 것을 막는다.
   실제로 결론 문단이 그렇게 끊겨 있었다. */
p{margin:0 0 14px; max-width:66ch; color:var(--ink-2); text-wrap:pretty}
...
/* 계수 차트는 2 x 2. 예전에는 auto-fit minmax(330px) 라 1120 px 폭에서
   3 칸이 되고 넷째가 혼자 한 줄을 써서 오른쪽 3 분의 2 가 비었다.
   2 칸이면 각 그림이 두 배 넓어져 곡선이 훨씬 잘 읽힌다. */
.charts{display:grid; grid-template-columns:repeat(auto-fit,minmax(440px,1fr));
...
function axes(c, w, h, xd, yd, xl, yl, title){
  // 여백. y 이름을 세로로 쓰지 않게 되어 왼쪽을 56 -> 46 으로 줄였다.
  // 그만큼 그림이 넓어진다. 위쪽은 제목 + 단위 줄을 넣느라 26 -> 34.
  const P = {l: 46, r: 14, t: 34, b: 40};
  ...
  // ★ y 이름을 **가로로** 그림 위 오른쪽에 둔다. 예전에는 -90도로 돌려
  //   왼쪽 끝(x=13)에 그렸는데, 눈금 숫자(오른쪽 정렬로 x=48 에서 왼쪽으로
  //   뻗는다)와 겹치고 캔버스 밖으로 잘렸다. 한글은 세로로 읽기도 어렵다.
  if (yl){
    c.textAlign = "right"; c.textBaseline = "top";
    c.fillStyle = css("--ink-3");
    c.font = '400 11px "IBM Plex Mono", monospace';
    c.fillText(yl, w - P.r, 6);
  }
  if (title){
    c.fillStyle = css("--ink"); c.textAlign = "left"; c.textBaseline = "top";
    c.font = '600 13px "IBM Plex Sans", sans-serif';
    c.fillText(title, P.l - 34, 4);
  }
  return [X, Y];
}
