...
def report(params, trims, rows, out_txt="results/flight_envelope.txt"):
    ...
    A("")
    A("  [⚠ 정정 2026-09-09] 여유비 < 1 을 '성립 안 함'으로 읽으면 안 된다.")
    A("    이 여유비는 **초기 옆미끄럼**에서 한 번 잰 값이다. 그런데 외란 모멘트는")
    A("    beta 에 비례하므로, 기수가 도는 만큼 외란이 **스스로 줄어든다**(자기제한).")
    A("    제어기를 태워 지속 측풍을 실제로 걸어 보면 발산하지 않고 정착한다")
    A("    (70 m/s, 측풍 15 m/s, 지속):")
    A("        완전 풍향계(포기) = 12.09 deg")
    A("        ScheduledLQR      = 11.64 deg  (남은 beta 0.03 -> 사실상 포기)")
    A("        NMPC+INDI 분리    =  8.30 deg  (남은 beta 2.08 -> 30% 회복)")
    A("    따라서 올바른 판정은 '못 이긴다'가 아니라")
    A("    **'지령 방위를 정확히는 못 지키고, 부분적으로 돌아간 각도에서 정착한다'** 이고,")
    A("    그 각도가 임무 요구 안인지가 기준이다. => 허용 기수오차 사양 확정 필요.")
    A("    재현: control/plot_proposal_figures.py 의 fig_crosswind()")
