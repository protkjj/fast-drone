...
def validate_config(config):
    ...
    for label, spec in config['controllers'].items():
        ...
        if floor is not None and (label != 'M17' or floor not in ('positive_thrust', 'positive_thrust_per_node')):
            raise ValueError(f"{label}: rotor_floor can only be 'positive_thrust' or 'positive_thrust_per_node' "
                             'on M17 (the only NMPC that plans rotor speeds)')
